...
def get_and_print_inference_time(model):
    for input_size in [224, 500]:
        for batch_size in [1, 10, 32]:
            ...
            print(f"Inference: input_size={input_size}, batch_size={batch_size}: cpu: {cpu_time}s" + ("gpu: {gpu_time}s" if torch.cuda.is_available() else ""))
...
def benchmark_model(model_name, measure_inference, verbose):
    benchmark_model_per_num_classes(model_name, 1, measure_inference)
